...
void cx_log_cat_set(const char *s_cat, int min_level) {
	if (!s_cat) {
		...
		CX_DBG_LOG_FMT(CX_LOG_CAT_LOGGING, "Log category minimum level set: (Global) -> %d\n", min_level);
		return;
	}

	struct cx_log_cat* p_cat = get_log_cat(s_cat, 0);

	if (p_cat->s_display_str == 0) {
		*p_cat = (struct cx_log_cat) {
			.s_display_str = p_log_cat_str_buf_next,
			.display_str_len = strlen(s_cat),
			.min_level = min_level
		};
		...
		*p_log_cat_str_buf_next++ = '\0';
		...
		CX_DBG_LOG_FMT(CX_LOG_CAT_LOGGING, "New log category: '%s' min_level=%d\n", s_cat, min_level);
	} else {
		p_cat->min_level = min_level;
		CX_DBG_LOG_FMT(CX_LOG_CAT_LOGGING, "Log category minimum level set: '%s' -> %d\n", s_cat, min_level);
	}
}
...
struct cx_log_cat* get_log_cat(const char* s_cat, size_t cat_len) {
	...
	while (p_cat->s_display_str) {
		const int b_match =
			(cat_len == 0 || cat_len == p_cat->display_str_len) &&
			p_cat->display_str_len != cat_len ?
...
int is_log_visible(const char* s_cat, int level) {
	...
	while (1) {
		if (*p == CX_LOG_CAT_DELIM || *p == '\0') {
			const struct cx_log_cat* p_cat = get_log_cat(s_cat, len);
			const int cat_min_level = p_cat->s_display_str ? p_cat->min_level : CX_LOG_ALL;
