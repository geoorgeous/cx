...
void transform_reset_local(struct transform* p_transform) {
    ...
    if (p_transform->p_local_transform) {
        vec3_copy(p_transform->p_local_transform->world_position, p_transform->world_position);
        vec_copy(4, p_transform->p_local_transform->world_rotation, p_transform->world_rotation);
        vec3_copy(p_transform->p_local_transform->world_scale, p_transform->world_scale);
    }
}
...
void transform_compute_world_trs_matrix(struct transform* p_transform) {    
    if (p_transform->p_local_transform) {
        ...
        vec3_copy(p_transform->position, p_transform->world_position);
        vec_copy(4, p_transform->rotation, p_transform->world_rotation);
        vec3_copy(p_transform->scale, p_transform->world_scale);
    }
...
void transform_set_world_position(struct transform* p_transform, const float* p_position) {
    if (p_transform->p_local_transform) {
        ...
        vec3_copy(p_position, p_transform->position);
    }
    vec3_copy(p_position, p_transform->world_position);
}
...
void transform_set_world_scale(struct transform* p_transform, const float* p_scale) {
    if (p_transform->p_local_transform) {
        ...
        vec3_copy(p_scale, p_transform->scale);
    }
    vec3_copy(p_scale, p_transform->world_scale);
}
