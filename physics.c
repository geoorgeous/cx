...
static void physics_collider_find_extreme(const struct physics_collider* p_collider, const float* p_axis, float* p_result);
static void physics_sphere_find_extreme(const struct physics_collider* p_collider, const float* p_axis, float* p_result);
static void physics_capsule_find_extreme(const struct physics_collider* p_collider, const float* p_axis, float* p_result);
static void physics_hull_find_extreme(const struct physics_collider* p_collider, const float* p_axis, float* p_result);
static void physics_collider_find_support(const struct physics_collider* p_a, const struct physics_collider* p_b, const float* p_axis, float* p_result);

static int  gjk(const struct physics_collider* p_a, const struct physics_collider* p_b, const float* p_ab);
static void gjk_test_simplex_line(float simplex[4][3], float* p_axis, size_t* p_i);
static void gjk_test_simplex_triangle(float simplex[4][3], float* p_axis, size_t* p_i);
static void gjk_test_simplex_tetrahedron(float simplex[4][3], float* p_axis, size_t* p_i);
static void epa(float simplex[4][3], const struct physics_collider* p_a, const struct physics_collider* p_b, float* p_normal, float* p_depth);
...
void physics_world_init(struct physics_world* p_world) {
    ...
    log_msg(LOG_TRACE, "physics", "Physics world initialised\n");
}
...
void physics_world_new_object_collider(struct physics_world* p_world, struct physics_object* p_object, enum physics_collider_type type) {
    ...
    log_msg(LOG_TRACE, "physics", "Collider added to %s (type=%d)\n", p_object->_b_is_rigidbody ? "rigidbody" : "static body", type);
}
...
void physics_world_step(struct physics_world* p_world, float delta_time) {
    //log_msg(LOG_DEBUG, "physics", "Step (delta_time=%f)\n", delta_time);
...
int physics_test_sphere_sphere_internal(const float* p_center_a, float radius_a, const float* p_center_b, float radius_b, struct physics_collision_result* p_result) {
    ...
    if (d > radius_ab * radius_ab) {
        *p_result = (struct physics_collision_result){0};
        // log_msg(LOG_DEBUG, "physics", "Sphere-sphere no collision: a=([%f, %f, %f], %f) b=([%f, %f, %f], %f), dist=%.12f, a.radius+b.radius=%.12f, dist<(radius_ab*radius_ab)=%u\n"
    ...
    // log_msg(LOG_DEBUG, "physics", "Sphere-sphere collision detected: a=([%f, %f, %f], %f) b=([%f, %f, %f], %f), dist=%.12f, a.radius+b.radius=%.12f, dist<(a.radius+b.radius)=%u, result.a=[%f, %f, %f], result.b=[%f, %f, %f], result.norm=[%f, %f, %f], result.depth=%f\n"
    //     , p_center_a[0], p_center_a[1], p_center_a[2], radius_a
    //     , p_center_b[0], p_center_b[1], p_center_b[2], radius_b
    //     , d
    //     , radius_a + radius_b
...
int physics_test_collision_sphere_plane(
    ...
    // log_msg(LOG_DEBUG, "physics", "Sphere-plane collision detected: a=([%f, %f, %f], %f) b=([%f, %f, %f], %f), dist=%.12f, result.a=[%f, %f, %f], result.b=[%f, %f, %f], result.norm=[%f, %f, %f], result.depth=%f\n"
...
int physics_test_collision_hull_plane(
    ...
    physics_hull_find_extreme(p_a, p_result->ab_normal, p_result->a);
...
void physics_collision_solver_impulse(const struct physics_collision* p_collisions, size_t n, float delta_time) {
    for (size_t i = 0; i < n; ++i) {
        ...
        if (FLT_CMP(invmass_a, 0) && FLT_CMP(invmass_b, 0)) {
            log_msg(LOG_WARNING, "physics", "Zero-mass collision detected.\n");
            continue;
        }
...
void physics_collision_solver_smooth_positions(const struct physics_collision* p_collisions, size_t n, float delta_time) {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        //log_msg(LOG_DEBUG, 0, "Smooth positions solver: delta_a=[%f, %f, %f], delta_b=[%f, %f, %f]\n", p_deltas[0], p_deltas[1], p_deltas[2], p_deltas[3], p_deltas[4], p_deltas[5]);
...
int physics_test_gjk(
    ...
    float ab[3];
    vec3_sub(p_tb->world_position, p_ta->world_position, ab);
    vec3_norm(ab, ab);
    ...
    log_msg(LOG_DEBUG, 0, "GJK COLLISION DETECTED\n");

    epa(simplex, p_a, p_b, p_result->ab_normal, &p_result->depth);
    return 1;
}

typedef void(*physics_collider_find_extreme_func)(const struct physics_collider*, const float*, float*);

void physics_collider_find_extreme(const struct physics_collider* p_collider, const float* p_axis, float* p_result) {
    static const physics_collider_find_extreme_func func_table[] = {
        physics_sphere_find_extreme,
        physics_capsule_find_extreme,
        physics_hull_find_extreme
    };
    func_table[p_collider->type](p_collider, p_axis, p_result);
}

void physics_sphere_find_extreme(const struct physics_collider* p_collider, const float* p_axis, float* p_result) {
    vec3_mul_s(p_axis, p_collider->as_sphere.radius, p_result);
    vec3_add(p_collider->as_sphere.center, p_result, p_result);
}

void physics_capsule_find_extreme(const struct physics_collider* p_collider, const float* p_axis, float* p_result) {
    // todo:
}

void physics_hull_find_extreme(const struct physics_collider* p_collider, const float* p_axis, float* p_result) {
    float dmax = -FLT_MAX;
    float d;
    for (size_t i = 0; i < p_collider->as_hull.verts._length; ++i) {
        float* p_v = darr_get(&p_collider->as_hull.verts, i);
        d = vec3_dot(p_v, p_axis);
        log_msg(LOG_DEBUG, 0, "Finding hull extreme: v=[%f, %f, %f], d=%f\n", p_v[0], p_v[1], p_v[2], d);
        if (d > dmax) {
            dmax = d;
            vec3_set(p_v, p_result);
        }
    }
}

void physics_collider_find_support(const struct physics_collider* p_a, const struct physics_collider* p_b, const float* p_axis, float* p_result) {
    float tmp[3];
    
    vec3_inv(p_axis, tmp);
    physics_collider_find_extreme(p_b, tmp, p_result);

    physics_collider_find_extreme(p_a, p_axis, tmp);

    log_msg(LOG_DEBUG, 0, "Finding support: dir=[%f, %f, %f], a=[%f, %f, %f], b=[%f, %f, %f]\n", p_axis[0], p_axis[1], p_axis[2], tmp[0], tmp[1], tmp[2], p_result[0], p_result[1], p_result[2]);

    vec3_sub(tmp, p_result, p_result);
}

void gjk_test_simplex_line(float simplex[4][3], float* p_axis, size_t* p_i) {
    vec3_sub(simplex[1], simplex[0], simplex[2]);
    vec3_inv(simplex[0], simplex[3]);
    if (vec3_dot(simplex[2], simplex[3]) > 0) {
        vec3_cross(simplex[2], simplex[3], simplex[3]);
        vec3_cross(simplex[3], simplex[2], p_axis);
    } else {
        --(*p_i);
        vec3_set(simplex[3], p_axis);
    }
}

void gjk_test_simplex_triangle(float simplex[4][3], float* p_axis, size_t* p_i) {
    float ab[3];
    ...
    vec3_cross(normal, ac, cross);

    if (vec3_dot(cross, simplex[3]) > 0) {
        if (vec3_dot(ac, simplex[3]) > 0) {
            vec3_set(simplex[2], simplex[1]);
            --(*p_i);
            vec3_cross(ac, simplex[3], cross);
            vec3_cross(cross, ac, p_axis);

            return;
        }

        --(*p_i);
        gjk_test_simplex_line(simplex, p_axis, p_i);

        return;
    }

    vec3_cross(ab, normal, cross);
    if (vec3_dot(cross, simplex[3]) > 0) {
        --(*p_i);
        gjk_test_simplex_line(simplex, p_axis, p_i);
        return;
    ...
    if (vec3_dot(normal, simplex[3]) > 0) {
        vec3_set(normal, p_axis);
        return;
    }

    vec3_set(simplex[2], simplex[3]);
    vec3_set(simplex[1], simplex[2]);
    vec3_set(simplex[3], simplex[1]);
    vec3_inv(normal, p_axis);
}

void gjk_test_simplex_tetrahedron(float simplex[4][3], float* p_axis, size_t* p_i) {
    float abc_norm[3];
    float acd_norm[3];
    float adb_norm[3];
    float ao[3];

    vec3_sub(simplex[1], simplex[0], ao);
    vec3_sub(simplex[2], simplex[0], acd_norm);
    vec3_cross(ao, acd_norm, abc_norm);

    vec3_sub(simplex[3], simplex[0], adb_norm);
    vec3_cross(acd_norm, adb_norm, acd_norm);

    vec3_cross(adb_norm, ao, adb_norm);

    vec3_inv(simplex[0], ao);

    if (vec3_dot(abc_norm, ao) > 0) {
        --(*p_i);
        gjk_test_simplex_triangle(simplex, p_axis, p_i);
    } else if (vec3_dot(acd_norm, ao) > 0) {
        --(*p_i);
        vec3_set(simplex[2], simplex[1]);
        vec3_set(simplex[3], simplex[2]);
        gjk_test_simplex_triangle(simplex, p_axis, p_i);
    } else if (vec3_dot(adb_norm, ao) > 0) {
        --(*p_i);
        vec3_set(simplex[1], simplex[2]);
        vec3_set(simplex[3], simplex[1]);
        gjk_test_simplex_triangle(simplex, p_axis, p_i);
...
int gjk(const struct physics_collider* p_a, const struct physics_collider* p_b, const float* p_ab) {
    log_msg(LOG_DEBUG, 0, "=== GJK BEGIN ===\n");
    ...
    physics_collider_find_support(p_a, p_b, p_ab, simplex[0]);
    
    log_msg(LOG_DEBUG, 0, "...support_point=[%f, %f, %f], i=%d\n", simplex[0][0], simplex[0][1], simplex[0][2], i);
    
    ++i;


    float axis[3];
    vec3_inv(simplex[0], axis);
    vec3_norm(axis, axis);

    while (1) {
        physics_collider_find_support(p_a, p_b, axis, simplex[i]);
        
        log_msg(LOG_DEBUG, 0, "...support_point=[%f, %f, %f], i=%d, dot=%f\n", simplex[i][0], simplex[i][1], simplex[i][2], i, vec3_dot(simplex[i], axis));

        if (vec3_dot(simplex[i], axis) <= 0) {
            return 0;
        }
        ...
        if (i == 2) {
            gjk_test_simplex_line(simplex, axis, &i);
        } else if (i == 3) {
            gjk_test_simplex_triangle(simplex, axis, &i);
        } else {
            gjk_test_simplex_tetrahedron(simplex, axis, &i);
        }
    ...
    log_msg(LOG_DEBUG, 0, "=== GJK END ===\n");
}

void epa(float simplex[4][3], const struct physics_collider* p_a, const struct physics_collider* p_b, float* p_normal, float* p_depth) {
    *p_depth = FLT_MAX;

    while (FLT_CMP(*p_depth, FLT_MAX)) {
        float support[3];
        physics_collider_find_support(p_a, p_b, p_normal, support);

        float sqdist = vec3_dot(p_normal, support);

        if (fabsf(sqdist - *p_depth) > 0.001f) {
            *p_depth = FLT_MAX;
